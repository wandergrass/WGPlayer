...
open class BMPlayerLayerView: UIView {
    ...
    open lazy var player: AVPlayer? = {
        if let item = self.playerItem {
            ...
            self.snapshotOutput = AVPlayerItemVideoOutput(pixelBufferAttributes: nil)
            playerItem?.remove(self.snapshotOutput!)
            playerItem?.add(self.snapshotOutput!)
    ...
    open var videoGravity = AVLayerVideoGravity.resizeAspect {
     ...
     open var snapshotOutput: AVPlayerItemVideoOutput?
    ...
    open func seek(to secounds: TimeInterval, completion:(()->Void)?) {
        ...
        setupTimer()
        if self.player?.currentItem?.status == AVPlayerItem.Status.readyToPlay {
            let draggedTime = CMTime(seconds: secounds, preferredTimescale: 1)
